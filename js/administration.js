...
const renderTable = () => {
  ...
  const filtered = invitationRows.filter(row => {
    ...
    const inviter = row.invited_by_name || row.invited_by || '';
    return statusMatch && (!search || row.email.toLowerCase().includes(search) || inviter.toLowerCase().includes(search));
  });
  ...
  body.innerHTML = filtered.map(row => {
    ...
    const resendButton = ['failed', 'expired'].includes(row.status)
      ? `<button type="button" data-resend-id="${escapeHtml(row.id)}" data-resend-email="${escapeHtml(row.email)}">Resend</button>`
      : '';

    return `<tr>
      <td>${escapeHtml(row.email)}</td>
      <td><span title="${escapeHtml(row.invited_by || '')}">${escapeHtml(row.invited_by_name || 'Unknown User')}</span></td>
      <td>${formatDate(row.created_at)}</td>
      <td>${formatDate(row.sent_at)}</td>
      <td><span class="status-chip status-${escapeHtml(row.status)}">${escapeHtml(row.status)}</span></td>
      <td>${escapeHtml(detail)}</td>
      <td><div class="table-actions">${resendButton}<button type="button" data-delete-id="${escapeHtml(row.id)}" data-delete-email="${escapeHtml(row.email)}">Delete</button></div></td>
    </tr>`;
  }).join('');
};
...
const loadInvitations = async () => {
  ...
  if (inviterIds.length) {
    const { data: profiles, error: profileError } = await window.DND.client
      .from('profiles')
    ...
    if (!profileError) {
      (profiles || []).forEach(profile => {
        const name = profile.display_name || profile.username || 'Unknown User';
        profileMap.set(profile.id, profile.role ? `${name} (${profile.role})` : name);
      });
    }
  }
  ...
  invitationRows = rows.map(row => ({
    ...row,
    invited_by_name: profileMap.get(row.invited_by) || 'Unknown User'
  }));
...
const resendInvitation = async (id, email, button) => {
  ...
  const { error: deleteOldError } = await window.DND.client.from('invitations').delete().eq('id', id);
  window.DND.toast(deleteOldError ? 'Invitation resent, but the older history row could not be removed.' : `Invitation resent to ${email}.`, deleteOldError ? 'error' : 'success');
  await loadInvitations();
};

const deleteInvitation = async (id, email) => {
  const confirmed = await window.DNDModal.confirm({
    type: 'danger', kicker: 'Invitation Management', title: 'Delete Invitation',
    ...
    confirmText: 'Delete Invitation', cancelText: 'Keep Invitation', focusCancel: true
  });
  if (!confirmed) return;
